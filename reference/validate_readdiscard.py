...
# 排序後 present 一定在最前
expect('掛-present排最前', g[0]['present'], True)
expect('掛-absent排最後', g[-1]['present'], False)
...
for _ in range(500):
    ...
    # A) 向下壓：pressed 一定 = low_hit and high_hit
    for r in press_analyze(river):
        checks += 1
        if r['pressed'] != (r['low_hit'] and r['high_hit']):
            fails += 1
            print('[FAIL] 壓-pressed定義不符', r)

    # B) 六掛：排序後「present 且 first_turn」必單調不遞增(越前面越晚出現=越危險)
    g = gua_analyze(river)
    pres = [x for x in g if x['present']]
    checks += 1
    if any(pres[i]['first_turn'] < pres[i + 1]['first_turn'] for i in range(len(pres) - 1)):
        fails += 1
        print('[FAIL] 掛-排序非單調', [x['first_turn'] for x in pres])
    # present 一定全部排在 absent 前面
    checks += 1
    seen_absent = False
    ok = True
    for x in g:
    ...
    if not ok:
        fails += 1
        print('[FAIL] 掛-present未排在absent前', [(x['gua'], x['present']) for x in g])
